								...
								<div class="col-lg-4">
									<div class="page-header-breadcrumb">
										<ul class=" breadcrumb breadcrumb-title">
											...
											<li class="breadcrumb-item"><a href="plans_list">Contacts List</a>
											</li>
										</ul>
									</div>
								</div>
									...
									<div class="row">
										<div class="col-12">
											<div class="card">
												<!-- Choose User -->
												<div class="card-body">
													<div class="table-responsive" id="id_contact_list">
														Loading…
													</div>
												</div>
											</div>
...
$(document).ready(function() {
    find_contact_list();
    setInterval(find_contact_list, 60000); // Every 5 mins (300000), it will call
});

// To list the Templates from API
function find_contact_list() {
    $.ajax({
        type: 'post',
        url: "ajax/display_functions.php?call_function=contact_list",
        dataType: 'html',
        success: function(response) { // Succcess
            $("#id_contact_list").html(response);
        },
...
function call_getsingletemplate(message_content, total_mobile_no_count) {
    ...
    $("#id_modal_display").html(`<h5><strong>Message Content:</strong> <span>${message_content}</span></h5><h5><strong>Total Mobile Number:</strong> <span>${total_mobile_no_count}</span></h5><div id="newDiv"></div>`);
