...
async function ComposeSMPP(req, res) {
    try {
        ...
        /* {
  receiver_nos_path: '/var/www/html/ucp/uploads/compose_variables/1_file_1731923671565.csv',
  messages: 'Welcome',
  character_count: '7',
  request_id: '1_2024322152432_7906',
  ...
  templateName: 'tmpl name'
        ...
        const { receiver_nos_path, txt_sms_type, request_id, total_mobile_count, user_master_id, character_count, messages, user_id, rights_name, header, templateName } = req.body;

        let store_id, full_short_name;
        console.log("Rights Name : ", rights_name)
        console.log("WELCOME");
        ...
        // const check_available_credits = await db.query(
        //     `SELECT usr.user_master_id, lim.available_messages, usr.user_id, usr.user_short_name, usr.parent_id, usr.user_name  FROM user_management usr LEFT JOIN message_limit lim ON lim.user_id = usr.user_id WHERE usr.user_id = '${user_id}' AND usr.usr_mgt_status = 'Y'`
        // );
        ...
        full_short_name = (get_user_short_name.length === 0 || user_master == 1)
            ? user_short_name
            : `${get_user_short_name[0].user_short_name}_${user_short_name}`;
        ...
        const latest_compose = await db.query(`
                             SELECT compose_ucp_id, unique_compose_id 
                             FROM compose_smpp 
                            ORDER BY compose_ucp_id DESC 
                             LIMIT 1
                        `);
        ...
        // let get_tmpl_name = await db.query(`SELECT * FROM cm_content_template WHERE cm_sender_id = '${header}'`);
        // let tmplt_master_id = get_tmpl_name[0].tmplt_master_id;

        const insert_msg = `INSERT INTO compose_smpp VALUES(NULL, ${user_id},${rights_id},'${unique_compose_id}', ${total_mobile_count},'${header}','${templateName}' ,'${txt_sms_type}','${messages}', ${character_count}, 'W',CURRENT_TIMESTAMP, NULL, NULL, '${compose_unique_name}', '${receiver_nos_path}', NULL, 'N', NULL)`;
        ...
        if (user_master_id !== 1) {
            ...
            const update_query = `
    UPDATE message_limit ml
    JOIN rights_master rm ON ml.rights_id = rm.rights_id
    SET ml.available_messages = ml.available_messages - ${msg_mobile_credits}
    WHERE ml.user_id = '${user_id}' AND rm.rights_id = ${rights_id};
`;
