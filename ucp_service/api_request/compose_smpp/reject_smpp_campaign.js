...
async function Reject_Campaign(req) {
    ...
    try {
        ...
        get_campaign = await db.query(`SELECT total_mobile_no_count,ucp_status,message_content FROM compose_smpp WHERE ucp_status = 'W' and compose_ucp_id = '${compose_message_id}' `);
        ...
        if (get_campaign.length > 0) {
            ...
            update_campaign_set = await db.query(`UPDATE compose_smpp SET ucp_status = '${campaign_status}',reject_reason = '${reason}' WHERE ucp_status = 'W' and compose_ucp_id = '${compose_message_id}'`);
