...
async function SummaryReportSmpp(req) {
    try {
        ...
        let report_query = '', getsummary, filter_date_1;
        let total_response = [];
        ...
        if (user_master_id == 1) {
            report_query = `
                SELECT 
                    wht.user_id, usr.user_name, ussr.user_type, wht.campaign_name, 
                    DATE_FORMAT(wht.com_entry_date, '%d-%m-%Y') AS entry_date, wht.total_msg,
                    (CASE WHEN wht.report_status = 'Y' THEN wht.total_waiting ELSE 0 END) AS total_waiting,
                    (CASE WHEN wht.report_status = 'Y' THEN wht.total_process ELSE 0 END) AS total_process,
                    (CASE WHEN wht.report_status = 'Y' THEN wht.total_success ELSE 0 END) AS total_success,
                    (CASE WHEN wht.report_status = 'Y' THEN wht.total_failed ELSE 0 END) AS total_failed,
                    (CASE WHEN wht.report_status = 'Y' THEN wht.total_delivered ELSE 0 END) AS total_delivered,
                    (CASE WHEN wht.report_status = 'Y' THEN wht.total_read ELSE 0 END) AS total_read 
                FROM 
                    user_summary_report wht 
                LEFT JOIN 
                    user_management usr ON wht.user_id = usr.user_id 
                LEFT JOIN 
                    user_master ussr ON usr.user_master_id = ussr.user_master_id 
                WHERE  wht.rights_id = ${rights_id} AND
                    (DATE(wht.com_entry_date) BETWEEN '${date_range[0]}' AND '${date_range[1]}') 
                    AND wht.report_status = 'Y' 
                    ${filter_condition} 
                GROUP BY 
                    campaign_name 
                ORDER BY 
            ...
            report_query = `
                ...
                LEFT JOIN 
                    user_master ussr ON usr.user_master_id = ussr.user_master_id 
                WHERE 
                    (usr.user_id = '${user_id}' OR usr.parent_id IN (${user_id})) AND wht.rights_id = ${rights_id} 
                    AND (DATE(wht.com_entry_date) BETWEEN '${date_range[0]}' AND '${date_range[1]}') 
                    AND wht.report_status = 'Y' 
                    ${filter_condition} 
                GROUP BY 
                    campaign_name 
