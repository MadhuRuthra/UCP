...
async function View_Contact(req) {
	...
	try {
		...
		// query parameters
		logger_all.info("[View_Contact query parameters] : " + JSON.stringify(req.body));
		// insert_contact_grp to execute this query
		const get_grpname = await db.query(`SELECT * FROM contact_management where contact_mgt_id = '${contact_id}'`);
		...
		logger_all.info("[View_Contact failed response] : " + e)
