...
async function Approve_Composesms_List(req) {
    ...
    try {
        ...
        if (user_master_id.toString() === '1') {
            sms_query = `SELECT mt.*, um.user_name FROM compose_sms mt JOIN user_management um ON mt.user_id = um.user_id where mt.ucp_status = 'W' ORDER BY mt.compose_ucp_id DESC`;
        }
         else {
            sms_query = `SELECT mt.*, um.user_name FROM compose_sms mt JOIN user_management um ON mt.user_id = um.user_id WHERE mt.user_id = ${user_id} and mt.ucp_status = 'W' ORDER BY mt.compose_ucp_id DESC`;
        }
