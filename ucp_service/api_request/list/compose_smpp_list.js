...
// ComposeSMPPList function - start
async function ComposeSMPPList(req) {
    ...
    try {
        ...
        let { user_id, user_master_id, rights_name } = req.body;
        let template_query = '';
...
let rights_id = get_rights_id[0].rights_id;
        ...
        if (user_master_id.toString() === '1') {
            template_query = `
                ...
                JOIN user_management um ON mt.user_id = um.user_id and mt.rights_id = ${rights_id}
...
// Export the ComposeSMPPList function for use in other modules
module.exports = {
    ComposeSMPPList
};