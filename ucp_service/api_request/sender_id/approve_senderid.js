...
const getHeaderFile = require('../whatsapp_process/getHeader');
const env = process.env
const main = require('../../logger');
const { use } = require("../whatsapp_process/route");
...
async function ApproveSenderId(req, res) {
    ...
    try {
        ...
        const { mobile_number, phone_number_id, whatsapp_business_acc_id, bearer_token } = req.body;
        var succ_template = [], failed_template = [];
        ...
        const check_user = await db.query(`SELECT * from whatsapp_config WHERE concat(country_code, mobile_no) = '${mobile_number}'`);

        console.log(JSON.stringify(check_user));
        // if user not available send error response to client
        if (check_user.length == 0) {
            logger.info("[API RESPONSE] " + JSON.stringify({ response_code: 0, response_status: 201, response_msg: 'Invalid senderid.', request_id: req.body.request_id }))
            await db.query(`UPDATE api_log SET response_status = 'F',response_date = CURRENT_TIMESTAMP, response_comments = 'Invalid senderid' WHERE request_id = '${req.body.request_id}' AND response_status = 'N'`);
            return ({ response_code: 0, response_status: 201, response_msg: 'Invalid senderid.', request_id: req.body.request_id });
        }
        else {
            ...
            if (check_user[0].whatspp_config_status == 'Y') {
                logger.info("[API RESPONSE] " + JSON.stringify({ response_code: 0, response_status: 201, response_msg: 'senderID already exists.', request_id: req.body.request_id }))
                await db.query(`UPDATE api_log SET response_status = 'F',response_date = CURRENT_TIMESTAMP, response_comments = 'SenderID already exists' WHERE request_id = '${req.body.request_id}' AND response_status = 'N'`);
                return ({ response_code: 0, response_status: 201, response_msg: 'SenderID already exists.', request_id: req.body.request_id });
            }  
            else {

                const check_senderid = await db.query(`SELECT * from whatsapp_config WHERE phone_number_id = '${phone_number_id}' or whatsapp_business_acc_id = '${whatsapp_business_acc_id}' or bearer_token = '${bearer_token}' `);

                if (check_senderid.length > 0) {

                    logger.info("[API RESPONSE] " + JSON.stringify({ response_code: 0, response_status: 201, response_msg: 'Credentials already exists', request_id: req.body.request_id }))
                    await db.query(`UPDATE api_log SET response_status = 'F',response_date = CURRENT_TIMESTAMP, response_comments = 'Credentials already exists' WHERE request_id = '${req.body.request_id}' AND response_status = 'N'`);
                    return ({ response_code: 0, response_status: 201, response_msg: 'Credentials already exists', request_id: req.body.request_id });
                }

                
                // update the phone number id, whatsapp_business_acc_id, bearer token for the given mobile number
                await db.query(`UPDATE whatsapp_config SET phone_number_id = '${phone_number_id}',whatsapp_business_acc_id = '${whatsapp_business_acc_id}',bearer_token = '${bearer_token}',whatspp_config_status = 'Y',whatspp_config_apprdate = CURRENT_TIMESTAMP,available_credit=200 WHERE concat(country_code, mobile_no) = '${mobile_number}' AND whatspp_config_status = 'N'`);
                ...
                if (select_id.length > 1) {
                    ...
                    await axios(get_temp)
                        .then(async function (response) {
                            ...
                            if (response.data.data.length == 0) {
                                ...
                                await db.query(`UPDATE api_log SET response_status = 'F',response_date = CURRENT_TIMESTAMP, response_comments = 'No templates available' WHERE request_id = '${req.body.request_id}' AND response_status = 'N'`);
                            ...
                            for (var i = 0; i < response.data.data.length; i++) {
                                ...
                                if (response.data.data[i].status = 'APPROVED') {
                                    ...
                                    var isMedia = false, mediaPlace, variable_status = 0;
                                    // check if the template is in our db
                                    const select_lang = await db.query(`SELECT * FROM message_template tmp LEFT JOIN master_language lan on lan.language_id = tmp.language_id WHERE tmp.template_name = '${response.data.data[i].name}' AND lan.language_code = '${response.data.data[i].language}' AND tmp.template_status = 'Y'`);
                                    ...
                                    if (select_lang.length != 0) {
                                        ...
                                        if (isMedia) {
                                            console.log("Coming GO ")
                                            console.log(response.data.data[i].components[mediaPlace]['example']['header_handle'][0] + "sdknskdns")
                                            // url of the media
                                            var h_file = await getHeaderFile(response.data.data[i].components[mediaPlace]['example']['header_handle'][0]);
                                            console.log(h_file + "h_file")
                                            if (h_file) {
                                                // send request to the api to get the header_handle
                                                var command = `curl -X POST \"${api_url}${h_file[0]}" \--header "Authorization: OAuth ${media_bearer}" \--header "file_offset: 0" \--data-binary @${h_file[1]}`
                                                ...
                                                child = exec(command, async function (error, stdout, stderr) {
                                                    ...
                                                    await axios(post_temp)
                                                        .then(async function (response) {
                                                            ...
                                                            await db.query(`UPDATE message_template SET template_response_id = '${response.data.id}',template_status = 'S' WHERE template_id = ${insert_new_usr.insertId}`);
                                                        ...
                                                        .catch(async function (error) {
                                                            ...
                                                            await db.query(`UPDATE message_template SET template_status = 'F' WHERE template_id = ${insert_new_usr.insertId}`);
                        ...
                        .catch(async function (error) {
                            ...
                            await db.query(`UPDATE api_log SET response_status = 'F',response_date = CURRENT_TIMESTAMP, response_comments = 'Error occurred while getting templates' WHERE request_id = '${req.body.request_id}' AND response_status = 'N'`);
                    ...
                    logger.info("[API RESPONSE] " + JSON.stringify({ response_code: 1, response_status: 200, response_msg: 'New SenderID', request_id: req.body.request_id }))
                    await db.query(`UPDATE api_log SET response_status = 'S',response_date = CURRENT_TIMESTAMP, response_comments = 'New user' WHERE request_id = '${req.body.request_id}' AND response_status = 'N'`);
